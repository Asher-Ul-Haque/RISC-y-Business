...
Simulator *initializeSimulator() {
  ...
  simulator->memoryManager =
      allocateMemory(getRegister(simulator->registerFile, "sp"));
  ...
  unsigned short programCounter =
      0; // define the program counter in the stack instead of the heap
  ...
  *simulator->programCounter = 0;
  ...
  simulator->executionManager = initializeExecutorManager(
      simulator->memoryManager, simulator->registerFile,
...
void loadProgram(Simulator *SIMULATOR, const char *BINARY_FILE_PATH) {
  ...
  loadBinaryToProgramMemory(SIMULATOR->memoryManager,
...
void runSimulation(Simulator *SIMULATOR) {
  ...
  while (!(*SIMULATOR->programCounter >= PROGRAM_MEMORY)) {
    //     Bit* instruction = getMemoryCell(SIMULATOR->memoryManager,
    //     SIMULATOR->memoryManager->programMemory[*SIMULATOR->programCounter/PROGRAM_COUNTER_SCALE_FACTOR].bits)->bits;
    ...
    // SIMULATOR->memoryManager->programMemory[*SIMULATOR->programCounter/PROGRAM_COUNTER_SCALE_FACTOR].bits)->bits;
    printMemoryCell(
        getMemoryCell(SIMULATOR->memoryManager,
                      SIMULATOR->memoryManager
                          ->programMemory[*SIMULATOR->programCounter /
                                          PROGRAM_COUNTER_SCALE_FACTOR]
                          .bits));
    Bit *instruction =
        getMemoryCell(SIMULATOR->memoryManager,
                      SIMULATOR->memoryManager
